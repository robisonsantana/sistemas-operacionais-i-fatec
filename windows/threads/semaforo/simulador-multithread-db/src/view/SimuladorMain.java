...
public class SimuladorMain {
	 ...
	 * transações com bancos de dados. Por ter uma quantidade grande de núcleos de processamentos e 
	 * threads, além de um bom algoritmo de escalonamento de threads, enquanto as threads fazem cálculos, 
	 * estes podem ocorrer simultaneamente, mas quando se faz a transação no banco de dados, esta deve ser 
	 * apenas uma thread por vez. Considere um conjunto de threads com IDs definidas na própria aplicação com 
	 * números iniciando em 1 e incrementando de um em um. As threads tem comportamento como segue:
		...
		Faça uma aplicação em Java que simule a situação de 21 Threads simultâneas, com exibição em console de 
		cada passo que a Thread está realizando. */
	public static void main(String[] args) {
		
		Semaphore mutex = new Semaphore(1);
		
		for(int id = 1; id < 22; id++) {
			Thread db = new SimuladorController(id, mutex);
			db.start();
		}
